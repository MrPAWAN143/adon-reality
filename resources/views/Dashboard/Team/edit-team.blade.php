        ...
        <div class="mx-auto py-6 px-8 sm:px-12 bg-adminFormBg rounded-md">
            ...
            <form id="updateTeamMember"
                ...
                <div class="md:col-span-2">
                    <label for="full_name" class="block font-semibold text-adminTextPrimary mb-1">Full Name</label>
                    <input id="full_name" name="full_name" type="text" value="{{ old('full_name', $teamMember->full_name) }}"
                        class="w-full border border-adminInputBorder rounded px-3 py-2 focus:border-adminPrimary focus:ring-adminPrimary">
                </div>

                {{-- Designation --}}
                <div>
                    <label for="position" class="block font-semibold text-adminTextPrimary mb-1">Designation</label>
                    <input id="position" name="position" type="text" value="{{ old('position', $teamMember->position) }}"
                        class="w-full border border-adminInputBorder rounded px-3 py-2 focus:border-adminPrimary focus:ring-adminPrimary">
                </div>
                ...
                <div>
                    ...
                    @if($teamMember->photo)
                    <div class="mt-2">
                        <img src="{{ asset($teamMember->photo) }}"
                ...
                {{-- Short Bio --}}
                <div class="md:col-span-2">
                    <label for="short_bio" class="block font-semibold text-adminTextPrimary mb-1">Short Bio</label>
                    <textarea id="short_bio" name="short_bio" rows="3"
                        class="forBorder w-full border border-adminInputBorder rounded px-3 py-2 focus:border-adminPrimary focus:ring-adminPrimary">{{ old('short_bio', $teamMember->short_bio) }}</textarea>
                </div>
...
<script type="module">
    ...
    $('#updateTeamMember').on('submit', function(e) {
        e.preventDefault();
        $('.container').hide();
        $('.loadingbtn').show();
    ...
    $('#deleteTeamMember').on('submit', function(e) {
        e.preventDefault();
    ...
    const url = {
        update: "{{ route('team.update', $teamMember->id) }}",
        list: "{{ route('team.list') }}",
        delete: "{{ route('team.delete', $teamMember->id) }}"
    };
</script>
