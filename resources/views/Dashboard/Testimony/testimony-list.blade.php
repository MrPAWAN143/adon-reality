    ...
    <div class="my-5">
        <div class="mx-auto py-6 px-8 bg-adminFormBg rounded-md">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-adminTextPrimary">Testimony List</h1>
            ...
            <div class="overflow-x-auto bg-white shadow rounded-lg">
                <table class="min-w-full divide-y divide-adminInputBorder">
                    ...
                    <tbody>
                        <tr>
                            <td colspan="6" class="px-4 py-3 text-center text-gray-500">No testimonies found.</td>
                        </tr>
                    </tbody>
                    @else
                    <tbody class="divide-y divide-adminInputBorder text-sm">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 font-medium">{{ $testimony->name }}</td>
                            <td class="px-4 py-3">
                                {{ $testimony->role ?? 'N/A' }}
                            </td>
                            <td class="px-4 py-3">
                                {{ $testimony->review_text ?? 'N/A' }}
                            </td>


                            <td class="px-4 py-3">
                                @if($testimony->photo)
                                <img src="{{ asset($testimony->photo) }}" alt="{{ $testimony->name }}"
                            ...
                            <td class="px-4 py-3">
                                {{-- clickable badge --}}
                                <button id="badge-{{ $testimony->id }}"
                                    type="button"
                                    data-id="{{ $testimony->id }}"
                                    data-route="{{ route('testimony.toggle-status', $testimony->id) }}"
                                    class="status-toggle inline-block px-2 py-1 text-xs rounded-full
                    {{ $testimony->is_active ? 'bg-green-100 text-green-800'
...
<script type="module">
    $(document).ready(function() {
        const deleteRoute = id =>
            "{{ route('testimony.delete', ':id') }}".replace(':id', id);
