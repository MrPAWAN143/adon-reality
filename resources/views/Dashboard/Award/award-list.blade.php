                ...
                <table class="min-w-full divide-y divide-adminInputBorder">
                    <thead class="bg-adminSidebar text-white text-sm">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold">Featured Image</th>
                            <th class="px-4 py-3 text-left font-semibold">Is Featured</th>
                    ...
                    <tbody>
                        <tr>
                            <td colspan="6" class="px-4 py-3 text-center text-gray-500">No awards found.</td>
                        </tr>
                    </tbody>
                    @else
                    <tbody class="divide-y divide-adminInputBorder text-sm">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                {{-- clickable badge --}}
                                <button id="badge-{{ $award->id }}"
                                    type="button"
                                    data-id="{{ $award->id }}"
                                    data-route="{{ route('award.toggle-featured', $award->id) }}"
                                    class="toggle-featured inline-block px-2 py-1 text-xs rounded-full
                    {{ $award->is_featured ? 'bg-yellow-100 text-yellow-800'
                                           : 'bg-gray-100 text-gray-800' }}">
                                    {{ $award->is_featured ? 'Featured' : 'Past Award' }}
                                </button>
                            </td>

                            <td class="px-4 py-3">
                                @if($award->featured_image)
                                <img src="{{ asset($award->featured_image) }}" alt="{{ $award->title }}"
                            ...
                            <td class="px-4 py-3">
                                ...
                                <button id="badge-{{ $award->id }}"
                                    type="button"
                                    data-id="{{ $award->id }}"
                                    data-route="{{ route('award.toggle-status', $award->id) }}"
                                    class="status-toggle inline-block px-2 py-1 text-xs rounded-full
                    {{ $award->is_active ? 'bg-green-100 text-green-800'
                            ...
                            <td class="px-4 py-3 space-x-1">
                                ...
                                <a href="{{ route('award.edit', $award->id) }}" class="inline-block bg-adminPrimary hover:bg-adminPrimaryHover text-white px-3 py-1 rounded text-xs">Edit</a>
                                <button type="button" data-id="{{ $award->id }}" class="deletebtn bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs">
                                    Delete
                                </button>
                            </td>
...
<script type="module">
    $(document).ready(function() {
        const deleteRoute = id =>
            "{{ route('award.delete', ':id') }}".replace(':id', id);
        ...
        $(function() {
            ...
            $('.toggle-featured').on('click', function() {
                ...
                id = $(this).data('id');
                $('#statusModal').removeClass('hidden');
                $('#statusModal p').text('Are you sure you want to change the featured status of this item?');
            });
