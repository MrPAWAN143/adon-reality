...
@section('metadata')
<title>Media and Event List</title>
@endsection
    ...
    <div class="my-5">
        <div class="mx-auto py-6 px-8 bg-adminFormBg rounded-md">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-adminTextPrimary">Media and Event List</h1>
                ...
                <table class="min-w-full divide-y divide-adminInputBorder">
                    <thead class="bg-adminSidebar text-white text-sm">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left font-semibold">is Active</th>
                    ...
                    <tbody class="divide-y divide-adminInputBorder text-sm">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                {{-- clickable badge --}}
                                <button id="badge-{{ $event->id }}"
                                    type="button"
                                    data-id="{{ $event->id }}"
                                    data-route="{{ route('event.toggle-featured', $event->id) }}"
                                    class="toggle-featured inline-block px-2 py-1 text-xs rounded-full
                    {{ $event->is_featured ? 'bg-yellow-100 text-yellow-800'
                                           : 'bg-gray-100 text-gray-800' }}">
                                    {{ $event->is_featured ? 'Featured' : 'Past Event' }}
                                </button>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                ...
                                <button id="badge-{{ $event->id }}"
                                    type="button"
                                    data-id="{{ $event->id }}"
                                    data-route="{{ route('event.toggle-status', $event->id) }}"
                    ...
                    {{ $event->is_active ? 'bg-green-100 text-green-800'
                            ...
                            <td class="px-4 py-3 space-x-1">
                                ...
                                <a href="{{ route('event.edit', $event->id) }}" class="inline-block bg-adminPrimary hover:bg-adminPrimaryHover text-white px-3 py-1 rounded text-xs">Edit</a>
                                <button type="button" data-id="{{ $event->id }}" class="deletebtn bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs">
                                    Delete
                                </button>
                            </td>
...
<script type="module">
    $(document).ready(function() {
        const deleteRoute = id =>
            "{{ route('event.delete', ':id') }}".replace(':id', id);
